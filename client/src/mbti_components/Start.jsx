...
export default function Start() {
  ...
  async function fetchData() {
    const resMbtiData = await fetch('http://localhost:8001/data/getdata');
    ...
    const resGetCounts = await fetch('http://localhost:8001/data/getcount');
    if (resGetCounts.status === 200) {
      const countData = await resGetCounts.json();
      // setCounts((cur) => countData.counts);
      if (countData.counts !== undefined) setCounts(countData.counts);
    } else {
  ...
  return (
    <>
      ...
      <MainImg src="/images/main.jpg" alt="main_mbti" />
      <SubHeader>
        개발자가 흔히 접하는 상황에 따라 MBTI를 알아 봅시다! {'\n\n'}
        {counts} 명이 참여하셨습니다.
      </SubHeader>
