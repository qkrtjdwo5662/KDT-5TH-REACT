...
const Login = () => {
  ...
  const [inputId, setId] = useState(document.querySelector('input-email'));
  return(
    <div className="container-lg d-flex flex-column align-items-center bg-light w-25 mt-5 p-5 rounded-3">
      ...
      <form action="/login" method="POST" className="w-75">
        <div className="mt-3 id">
          ...
          <input type="text" name="id" className="form-control" id="input-email" placeholder="아이디를 입력하세요" required />
        </div>
        ...
        <div className="d-flex justify-content-center mt-4 password">
          <button className="btn btn-primary" onClick={() => setId((cur)=>{cur.value==='park' ? console.log(true) : console.log({cur})})}>로그인</button>
        </div>
      </form>
    </div>
  )
